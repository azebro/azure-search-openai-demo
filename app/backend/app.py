...
AZURE_OPENAI_SERVICE = os.environ.get("AZURE_OPENAI_SERVICE") or "openai-test-adamze"
...
def ask():
    ensure_openai_token(False) #remove False line if not using keys
...
def ensure_openai_token(useToken=True):
    if useToken:
        global openai_token
